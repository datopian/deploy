...
k8s_connect_gke() {
	export DATOPIAN_CLOUDSDK_CORE_PROJECT
	export DATOPIAN_CLOUDSDK_CONTAINER_CLUSTER
	export DATOPIAN_CLOUDSDK_COMPUTE_ZONE
  ...
  export PS1="${K8S_CONNECT_ORIGINAL_PS1}\[\033[01;33m\]${DATOPIAN_CLOUDSDK_CORE_PROJECT}-${DATOPIAN_CLOUDSDK_CONTAINER_CLUSTER}-${K8S_NAMESPACE}\[\033[0m\]$ "
  source <(kubectl completion bash)
  echo "Connected to ${DATOPIAN_CLOUDSDK_CORE_PROJECT}-${DATOPIAN_CLOUDSDK_CONTAINER_CLUSTER}-${K8S_NAMESPACE}"
}
...
if [ -f .env ] && eval `dotenv -f ".env" list` \
   && [ "${DATOPIAN_CLOUDSDK_CORE_PROJECT}" != "" ] \
   && [ "${DATOPIAN_CLOUDSDK_CONTAINER_CLUSTER}" != "" ] \
   && [ "${DATOPIAN_CLOUDSDK_COMPUTE_ZONE}" != "" ] \
   && [ "${K8S_NAMESPACE}" != "" ] \
   ...
   && [ "${K8S_ENVIRONMENT_CONTEXT}" == "`kubectl config current-context 2>/dev/null`" ]; then
	k8s_connect_gke
elif [ "${DATOPIAN_CLOUDSDK_CORE_PROJECT}" == "" ] && [ "${DATOPIAN_CLOUDSDK_CONTAINER_CLUSTER}" == "" ] && [ "${DATOPIAN_CLOUDSDK_COMPUTE_ZONE}" == "" ]
then
    k8s_connect_custom
elif ! gcloud "--project=${DATOPIAN_CLOUDSDK_CORE_PROJECT}" \
            container clusters get-credentials "$DATOPIAN_CLOUDSDK_CONTAINER_CLUSTER" \
            "--zone=${DATOPIAN_CLOUDSDK_COMPUTE_ZONE}" >/dev/null 2>&1; then
    echo
